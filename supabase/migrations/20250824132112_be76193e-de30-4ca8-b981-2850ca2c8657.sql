-- Fix search path security issue by properly dropping and recreating with dependencies
DROP TRIGGER IF EXISTS on_auth_user_created_onboarding ON auth.users CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_user_onboarding() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_readiness(UUID) CASCADE;
