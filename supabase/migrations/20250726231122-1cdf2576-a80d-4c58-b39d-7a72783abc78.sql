-- Create investment products table (corrected with TEXT category_id to match existing table)
CREATE TABLE public.investment_products (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL,
  ria_id UUID NOT NULL,
  category_id TEXT REFERENCES public.investment_categories(id) ON DELETE SET NULL,
...
-- Create user product interests table (enhanced version of existing)
CREATE TABLE public.user_product_interests_marketplace (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
ALTER TABLE public.user_product_interests_marketplace ENABLE ROW LEVEL SECURITY;
...
-- RLS Policies for user_product_interests_marketplace
CREATE POLICY "Users can manage their own marketplace interests" ON public.user_product_interests_marketplace
  FOR ALL USING (
    tenant_id = get_current_user_tenant_id() AND 
    user_id = auth.uid()
  );

CREATE POLICY "RIAs can view interests for their products" ON public.user_product_interests_marketplace
  FOR SELECT USING (
    tenant_id = get_current_user_tenant_id() AND 
    EXISTS (
      SELECT 1 FROM public.investment_products ip 
      WHERE ip.id = user_product_interests_marketplace.product_id 
      AND (ip.ria_id = auth.uid() OR has_any_role(ARRAY['admin', 'tenant_admin', 'system_administrator']))
    )
  );
...
CREATE INDEX idx_user_interests_marketplace_user_product ON public.user_product_interests_marketplace(user_id, product_id);
CREATE INDEX idx_user_interests_marketplace_tenant ON public.user_product_interests_marketplace(tenant_id, interest_type);
...
CREATE TRIGGER trigger_user_product_interests_marketplace_updated_at
  BEFORE UPDATE ON public.user_product_interests_marketplace
  FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();
