-- Security Training and Reporting System
...
CREATE TABLE public.security_review_completions (
  ...
  review_subject TEXT NOT NULL, -- e.g., "PR #123", "Product Feature X"
...
TO authenticated
USING (tenant_id = get_current_user_tenant_id() AND has_any_role(ARRAY['admin', 'tenant_admin', 'security_officer']));
...
TO authenticated
USING (tenant_id = get_current_user_tenant_id() AND (user_id = auth.uid() OR has_any_role(ARRAY['admin', 'tenant_admin', 'security_officer'])));
...
TO authenticated
USING (tenant_id = get_current_user_tenant_id() AND (reported_by_user_id = auth.uid() OR has_any_role(ARRAY['admin', 'tenant_admin', 'security_officer'])));
...
TO authenticated
USING (tenant_id = get_current_user_tenant_id() AND (reviewed_by = auth.uid() OR has_any_role(ARRAY['admin', 'tenant_admin', 'security_officer'])));
...
-- Insert initial security training programs
INSERT INTO public.security_training_programs (program_name, program_type, description, duration_minutes, required_for_roles) VALUES
('Phishing Awareness Training', 'phishing_simulation', 'Learn to identify and report phishing attempts, social engineering tactics, and suspicious communications.', 45, ARRAY['all']),
('Zero Trust Security Principles', 'zero_trust_training', 'Understanding zero trust architecture, network segmentation, and identity verification principles.', 60, ARRAY['all']),
('Secure Development Practices', 'general_security', 'Security best practices for developers including secure coding, vulnerability testing, and code review processes.', 90, ARRAY['developer', 'tech_lead', 'admin']),
('Incident Response Procedures', 'incident_response', 'Step-by-step procedures for reporting and responding to security incidents.', 30, ARRAY['all']),
('Data Protection and Privacy', 'compliance_training', 'GDPR, HIPAA, and other data protection regulations compliance training.', 75, ARRAY['all']);

-- Insert quarterly training schedules
INSERT INTO public.security_training_schedules (program_id, schedule_name, frequency, next_due_date, target_audience, mandatory) 
SELECT id, program_name || ' - Quarterly Schedule', 'quarterly', CURRENT_DATE + INTERVAL '30 days', ARRAY['all_staff', 'contractors'], true
FROM public.security_training_programs;

-- Insert default security review checklists
INSERT INTO public.security_review_checklists (checklist_name, checklist_type, checklist_items, mandatory_items) VALUES
('Code Review Security Checklist', 'code_review', '[
  {"id": "input_validation", "item": "All user inputs are properly validated and sanitized", "required": true},
  {"id": "authentication", "item": "Authentication and authorization checks are implemented", "required": true},
  {"id": "sql_injection", "item": "Code is protected against SQL injection attacks", "required": true},
  {"id": "xss_protection", "item": "Code includes XSS protection mechanisms", "required": true},
  {"id": "secrets_management", "item": "No hardcoded secrets or credentials in code", "required": true},
  {"id": "error_handling", "item": "Error messages do not expose sensitive information", "required": false},
  {"id": "logging", "item": "Security events are properly logged", "required": false}
]', ARRAY['input_validation', 'authentication', 'sql_injection', 'xss_protection', 'secrets_management']),

('Product Planning Security Review', 'product_planning', '[
  {"id": "threat_model", "item": "Threat modeling has been performed for the feature", "required": true},
  {"id": "data_classification", "item": "Data sensitivity and classification has been determined", "required": true},
  {"id": "privacy_impact", "item": "Privacy impact assessment completed", "required": true},
  {"id": "compliance_review", "item": "Compliance requirements reviewed and addressed", "required": true},
  {"id": "security_testing", "item": "Security testing plan developed", "required": false},
  {"id": "incident_response", "item": "Incident response procedures updated if needed", "required": false}
]', ARRAY['threat_model', 'data_classification', 'privacy_impact', 'compliance_review']);
...
BEGIN
  ...
  RETURN CASE p_frequency
    ...
    ELSE base_date + INTERVAL '3 months' -- Default to quarterly
  END;
END;
$$;
