-- Multi-Persona OS Hardening Migration

-- 1. Add helper function has_any_role()
...
UPDATE public.personas SET persona_kind = persona_type::public.persona_kind;
...
-- Migrate existing component_ids array to junction table
...
ALTER TABLE public.ui_layouts DROP COLUMN component_ids;
...
-- Add unique constraint on (tenant_id, block_number)
ALTER TABLE public.persona_audit 
ADD CONSTRAINT persona_audit_tenant_block_unique UNIQUE (tenant_id, block_number);

-- 8. Create indexes for performance
CREATE INDEX idx_models_tenant_active ON public.models(tenant_id, is_active);
CREATE INDEX idx_persona_thresholds_tenant ON public.persona_thresholds(tenant_id);
CREATE INDEX idx_ui_layout_components_layout ON public.ui_layout_components(layout_id, position);
CREATE INDEX idx_policy_tokens_hash_alg ON public.policy_tokens(hash_alg, token_hash);
