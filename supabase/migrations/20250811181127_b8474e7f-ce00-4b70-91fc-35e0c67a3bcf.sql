-- Add missing tenant_id columns to existing tables
...
-- Create simplified RLS policies for tables that have tenant_id
CREATE POLICY "tenant_isolation_select" ON public.credential_sources
  FOR SELECT USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_insert" ON public.credential_sources
  FOR INSERT WITH CHECK (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_update" ON public.credential_sources
  FOR UPDATE USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

-- Apply similar policies to other tables
CREATE POLICY "tenant_isolation_select" ON public.sanction_hits
  FOR SELECT USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_insert" ON public.sanction_hits
  FOR INSERT WITH CHECK (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_update" ON public.sanction_hits
  FOR UPDATE USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_select" ON public.monitoring_jobs
  FOR SELECT USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_insert" ON public.monitoring_jobs
  FOR INSERT WITH CHECK (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_update" ON public.monitoring_jobs
  FOR UPDATE USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_select" ON public.reconciliation_logs
  FOR SELECT USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_insert" ON public.reconciliation_logs
  FOR INSERT WITH CHECK (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

CREATE POLICY "tenant_isolation_update" ON public.reconciliation_logs
  FOR UPDATE USING (tenant_id IS NULL OR tenant_id = public.current_tenant_id());

-- Trust score maintenance function
