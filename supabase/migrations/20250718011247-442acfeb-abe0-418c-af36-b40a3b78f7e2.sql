-- Fix critical tenant isolation security gaps (corrected)

-- 1. Make tenant_id NOT NULL for critical tables (skip if data exists)
...
-- 4. Tighten educational content policy (remove is_global reference)
...
-- 5. Fix training modules policy (correct column structure)
...
-- 6. Add constraints to prevent NULL tenant_id on critical tables
ALTER TABLE public.professionals ADD CONSTRAINT IF NOT EXISTS check_professionals_tenant_id CHECK (tenant_id IS NOT NULL);
ALTER TABLE public.documents ADD CONSTRAINT IF NOT EXISTS check_documents_tenant_id CHECK (tenant_id IS NOT NULL);