-- Create QA test users for each persona (corrected structure)
DO $$
DECLARE
    user_id UUID;
    test_password_hash TEXT := crypt('Test1234!', gen_salt('bf'));
BEGIN
    -- Temporarily disable triggers to avoid audit log conflicts
    ...
    INSERT INTO auth.users (
        ...
        '{"role":"client","subscription_tier":"basic"}'::jsonb
    );
    
    INSERT INTO public.profiles (
        id, email, role, subscription_tier, first_name, last_name, 
    ...
    INSERT INTO auth.users (
        ...
        now(),
        '{"provider":"email","providers":["email"]}'::jsonb,
        '{"role":"client","subscription_tier":"premium"}'::jsonb
    );
