-- Analytics tables for project performance tracking (corrected)
CREATE TABLE IF NOT EXISTS public.project_analytics (
  ...
  communication_frequency NUMERIC DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS public.team_productivity_analytics (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.resource_utilization_analytics (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL,
  period_start DATE NOT NULL,
  period_end DATE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_project_analytics_tenant_id ON public.project_analytics(tenant_id);
CREATE INDEX IF NOT EXISTS idx_project_analytics_calculated_at ON public.project_analytics(calculated_at);

CREATE INDEX IF NOT EXISTS idx_team_productivity_user_id ON public.team_productivity_analytics(user_id);
CREATE INDEX IF NOT EXISTS idx_team_productivity_tenant_id ON public.team_productivity_analytics(tenant_id);
CREATE INDEX IF NOT EXISTS idx_team_productivity_period ON public.team_productivity_analytics(period_start, period_end);

CREATE INDEX IF NOT EXISTS idx_resource_utilization_tenant_id ON public.resource_utilization_analytics(tenant_id);
CREATE INDEX IF NOT EXISTS idx_resource_utilization_period ON public.resource_utilization_analytics(period_start, period_end);

CREATE INDEX IF NOT EXISTS idx_analytics_dashboards_tenant_id ON public.analytics_dashboards(tenant_id);
...
DROP POLICY IF EXISTS "Users can view project analytics in their tenant" ON public.project_analytics;
CREATE POLICY "Users can view project analytics in their tenant"
  ON public.project_analytics FOR SELECT
  USING (tenant_id = get_current_user_tenant_id());

DROP POLICY IF EXISTS "System can manage project analytics" ON public.project_analytics;
CREATE POLICY "System can manage project analytics"
  ON public.project_analytics FOR ALL
  USING (tenant_id = get_current_user_tenant_id());

-- Team Productivity Policies
DROP POLICY IF EXISTS "Users can view team productivity in their tenant" ON public.team_productivity_analytics;
CREATE POLICY "Users can view team productivity in their tenant"
  ON public.team_productivity_analytics FOR SELECT
  USING (tenant_id = get_current_user_tenant_id());

DROP POLICY IF EXISTS "Users can view their own productivity analytics" ON public.team_productivity_analytics;
CREATE POLICY "Users can view their own productivity analytics"
  ON public.team_productivity_analytics FOR SELECT
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "System can manage team productivity analytics" ON public.team_productivity_analytics;
...
DROP POLICY IF EXISTS "Users can view resource utilization in their tenant" ON public.resource_utilization_analytics;
CREATE POLICY "Users can view resource utilization in their tenant"
  ON public.resource_utilization_analytics FOR SELECT
  USING (tenant_id = get_current_user_tenant_id());

DROP POLICY IF EXISTS "Admins can manage resource utilization analytics" ON public.resource_utilization_analytics;
...
DROP POLICY IF EXISTS "Users can view shared dashboards in their tenant" ON public.analytics_dashboards;
...
-- Update existing analytics trigger or create new one
DROP TRIGGER IF EXISTS projects_analytics_trigger ON public.projects;
DROP TRIGGER IF EXISTS tasks_analytics_trigger ON public.project_tasks;  
DROP TRIGGER IF EXISTS milestones_analytics_trigger ON public.project_milestones;
...
CREATE TRIGGER projects_analytics_trigger
  ...
  FOR EACH ROW EXECUTE FUNCTION public.trigger_project_analytics_update();
