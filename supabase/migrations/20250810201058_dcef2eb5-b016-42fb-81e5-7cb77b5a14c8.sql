-- Fix Portfolio Risk Navigator GPS Database Schema

-- Drop existing tables to recreate with correct schema
DROP TABLE IF EXISTS public.portfolio_positions CASCADE;
DROP TABLE IF EXISTS public.portfolio_targets CASCADE;
DROP TABLE IF EXISTS public.rebalancing_events CASCADE;
DROP TABLE IF EXISTS public.recommendation_audit CASCADE;
DROP TABLE IF EXISTS public.private_fund_holdings CASCADE;
DROP TABLE IF EXISTS public.manager_signals CASCADE;
...
CREATE TABLE public.rebalancing_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL,
  account_id UUID NOT NULL,
...
CREATE TABLE public.private_fund_holdings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL,
...
CREATE TABLE public.manager_signals (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_portfolio_positions_as_of_date ON public.portfolio_positions(as_of_date);
CREATE INDEX idx_portfolio_targets_user_phase ON public.portfolio_targets(user_id, phase);
CREATE INDEX idx_rebalancing_events_user_id ON public.rebalancing_events(user_id);
CREATE INDEX idx_private_fund_holdings_user_id ON public.private_fund_holdings(user_id);
CREATE INDEX idx_manager_signals_manager_id ON public.manager_signals(manager_id);
