-- IP Watch & Enforcement System Migration
-- Creates tables for monitoring, receipts, enforcement queue, and policies

-- Enable pgcrypto extension if not already enabled
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- IP Hits table for monitoring CPC clusters/keywords
CREATE TABLE public.ip_hits (
...
CREATE TABLE public.enforcement_queue (
    item_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    entity_id UUID,
    action TEXT NOT NULL,
    status TEXT DEFAULT 'pending',
    priority INTEGER DEFAULT 3,
    ref_hit_id UUID REFERENCES public.ip_hits(hit_id),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);

-- Policies table for versioned policy definitions
...
-- Performance indexes
CREATE INDEX idx_ip_hits_ts ON public.ip_hits(ts DESC);
CREATE INDEX idx_ip_hits_entity_id ON public.ip_hits(entity_id);
CREATE INDEX idx_receipts_root ON public.receipts(root);
CREATE INDEX idx_receipts_entity_id ON public.receipts(entity_id);
CREATE INDEX idx_receipts_created_at ON public.receipts(created_at DESC);
CREATE INDEX idx_enforcement_queue_status ON public.enforcement_queue(status);
CREATE INDEX idx_enforcement_queue_entity_id ON public.enforcement_queue(entity_id);
CREATE INDEX idx_enforcement_queue_priority ON public.enforcement_queue(priority DESC);
CREATE INDEX idx_policies_name_version ON public.policies(name, version);
...
-- Policies table policies (global read, admin write)
