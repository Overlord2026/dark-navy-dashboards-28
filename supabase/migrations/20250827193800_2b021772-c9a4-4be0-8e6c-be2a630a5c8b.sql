...
-- Optional fallback (only if you prefer uuid_generate_v4()):
-- CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Helper: JSONB defaults
-- NOTE: Always use ::jsonb for JSONB defaults and inserts.
...
CREATE TABLE IF NOT EXISTS public.rev_rules (
  ...
  basis TEXT NOT NULL,                 -- 'bps'|'flat'|'tiered'
...
CREATE TABLE IF NOT EXISTS public.automation_runs (
  ...
  status TEXT NOT NULL,                -- 'ok'|'failed'|'dryrun'
...
CREATE TABLE IF NOT EXISTS public.transition_emails (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  transition_id UUID NOT NULL REFERENCES public.transitions(id) ON DELETE CASCADE,
  step TEXT NOT NULL,                  -- 'reminder_1'|'reminder_2'|'final_3'
...
CREATE TABLE IF NOT EXISTS public.transition_email_queue (
  ...
  status TEXT NOT NULL DEFAULT 'pending',  -- 'pending'|'sent'|'failed'|'bounced'|'suppressed'|'opened'|'clicked'|'opted_out'
  last_error TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.transition_events (
  ...
  transition_id UUID NOT NULL REFERENCES public.transitions(id) ON DELETE CASCADE,
  contact_id UUID,
  event TEXT NOT NULL,                 -- 'letter_sent','email_sent','open','click','bounce','opt_out','objection','confirm','suppressed'
...
-- Placeholder permissive policies (replace with tenant-aware checks later)
DO $$
DECLARE t TEXT;
BEGIN
  FOR t IN
    SELECT unnest(ARRAY[
      'iar_sites','iar_pages','rev_rules','rev_ledger','rev_payouts',
      'automations','automation_runs','transitions','transition_accounts',
      'transition_contacts','transition_emails','transition_email_queue',
      'transition_events','diligence_cases','diligence_artifacts','vault_objects'
    ])
  LOOP
    EXECUTE format($f$
      CREATE POLICY IF NOT EXISTS p_%1$s_all ON public.%1$s
