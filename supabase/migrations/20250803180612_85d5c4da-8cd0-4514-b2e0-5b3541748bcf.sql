...
CREATE TABLE IF NOT EXISTS public.accountant_clients (
  ...
  accountant_user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
...
FOR ALL USING (accountant_user_id = auth.uid());
...
FOR ALL USING (EXISTS (
  ...
  AND audit_checklists.accountant_user_id = auth.uid()
));
...
FOR ALL USING (EXISTS (
  ...
  AND accountant_invoices.accountant_user_id = auth.uid()
));
