-- Check if tenants table exists and fix conflicts
DO $$
BEGIN
...
DROP TRIGGER IF EXISTS update_fee_models_updated_at ON public.fee_models;
CREATE TRIGGER update_fee_models_updated_at BEFORE UPDATE ON public.fee_models
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_fee_scenarios_updated_at ON public.fee_scenarios;
...
DROP POLICY IF EXISTS "tenants manageable by owners/admins" ON public.tenants;
DROP POLICY IF EXISTS "tenants readable by public for branding" ON public.tenants;
DROP POLICY IF EXISTS "memberships by user" ON public.tenant_memberships;
DROP POLICY IF EXISTS "memberships manageable by admins" ON public.tenant_memberships;
DROP POLICY IF EXISTS "fee_models read by tenant members" ON public.fee_models;
DROP POLICY IF EXISTS "fee_models write by admins" ON public.fee_models;
DROP POLICY IF EXISTS "fee_scenarios read/write by owner or tenant admin" ON public.fee_scenarios;
DROP POLICY IF EXISTS "fee_demo_presets readable by all" ON public.fee_demo_presets;
DROP POLICY IF EXISTS "fee_analytics_events by tenant" ON public.fee_analytics_events;
DROP POLICY IF EXISTS "fee_analytics_events insert public" ON public.fee_analytics_events;
...
CREATE POLICY "fee_models read by public" ON public.fee_models
  FOR SELECT USING (true);
...
-- Storage policies
DROP POLICY IF EXISTS "Tenant branding files are publicly accessible" ON storage.objects;
DROP POLICY IF EXISTS "Tenant admins can upload branding files" ON storage.objects;
DROP POLICY IF EXISTS "Tenant admins can update their branding files" ON storage.objects;
DROP POLICY IF EXISTS "Tenant admins can delete their branding files" ON storage.objects;
