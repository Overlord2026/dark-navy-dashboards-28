-- 2025-08-17_security_hardening_guarded_final.sql
-- Idempotent security hardening for Boutique Family Office™ (Final Safe Version)
...
-- 3) Owner-based baseline policies where user_id exists
do $$
declare r record;
begin
  for r in
...
-- 4) Storage policies per bucket (safe naming)
do $$
declare b record; policy_name text;
begin
  for b in select id from storage.buckets loop
    policy_name := 'bfo_storage_' || translate(b.id, '-', '_') || '_read';
    begin
      execute format('create policy %I on storage.objects for select using (bucket_id=%L and (metadata->>''owner'')::uuid = auth.uid());',
                     policy_name, b.id);
    exception when duplicate_object then null; end;
    
    policy_name := 'bfo_storage_' || translate(b.id, '-', '_') || '_write';
    begin
      execute format('create policy %I on storage.objects for insert with check (bucket_id=%L and (metadata->>''owner'')::uuid = auth.uid());',
                     policy_name, b.id);
    exception when duplicate_object then null; end;
  end loop;
...
-- 5) SECURITY DEFINER hardening: set explicit search_path
do $$
declare f record;
begin
  for f in
