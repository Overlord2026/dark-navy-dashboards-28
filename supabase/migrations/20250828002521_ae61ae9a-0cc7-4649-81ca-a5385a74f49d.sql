...
-- Only allow authenticated users to view their own reserved profiles
CREATE POLICY "Users can view their own reserved profiles"
ON public.reserved_profiles FOR SELECT
TO authenticated
USING (user_id = auth.uid());

-- Allow users to manage their own reserved profiles
CREATE POLICY "Users can manage their own reserved profiles"
