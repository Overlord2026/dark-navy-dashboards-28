-- Add composite indexes with correct column names
...
-- Webhook Deliveries: Response status + Attempt for failure handling (using correct columns)
CREATE INDEX IF NOT EXISTS idx_webhook_deliveries_status_attempt 
ON public.webhook_deliveries(response_status, attempt_number, created_at);

-- Webhook Deliveries: Config + Status for monitoring
CREATE INDEX IF NOT EXISTS idx_webhook_deliveries_config_status_date 
ON public.webhook_deliveries(webhook_config_id, response_status, created_at);
...
-- Advisor Applications: Tenant + Status for application management
