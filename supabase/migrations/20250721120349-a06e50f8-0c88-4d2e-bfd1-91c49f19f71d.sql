-- Cross-Platform Integration System (avoiding existing tables)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_api_integrations_tenant_id') THEN
    CREATE INDEX idx_api_integrations_tenant_id ON public.api_integrations(tenant_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_api_integrations_status') THEN
    CREATE INDEX idx_api_integrations_status ON public.api_integrations(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_tool_connectors_tenant_id') THEN
    CREATE INDEX idx_tool_connectors_tenant_id ON public.tool_connectors(tenant_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_sync_jobs_tenant_id') THEN
    CREATE INDEX idx_sync_jobs_tenant_id ON public.sync_jobs(tenant_id);
  END IF;
END
$$;

-- RLS Policies (with safe drops)
...
DROP POLICY IF EXISTS "Users can manage integrations in their tenant" ON public.api_integrations;
...
DROP POLICY IF EXISTS "Users can manage tool connectors in their tenant" ON public.tool_connectors;
CREATE POLICY "Users can manage tool connectors in their tenant"
  ON public.tool_connectors FOR ALL
  USING (tenant_id = get_current_user_tenant_id());

-- Sync Jobs Policies
DROP POLICY IF EXISTS "Users can view sync jobs in their tenant" ON public.sync_jobs;
CREATE POLICY "Users can view sync jobs in their tenant"
  ON public.sync_jobs FOR SELECT
  USING (tenant_id = get_current_user_tenant_id());

-- Integration Templates Policies (public read)
DROP POLICY IF EXISTS "Anyone can view integration templates" ON public.integration_templates;
CREATE POLICY "Anyone can view integration templates"
  ON public.integration_templates FOR SELECT
  USING (true);

-- Integration Usage Policies
DROP POLICY IF EXISTS "Users can view integration usage for their tenant" ON public.integration_usage;
