...
-- Tables for connectors, accounts, positions, transactions, exceptions, billing
...
-- Add persona_scope and evidence_id to existing reports table
ALTER TABLE public.reports ADD COLUMN IF NOT EXISTS persona_scope TEXT CHECK (persona_scope IN ('client', 'advisor', 'family_office', 'institution'));
ALTER TABLE public.reports ADD COLUMN IF NOT EXISTS evidence_id UUID;
...
CREATE INDEX idx_reports_persona_scope ON public.reports(persona_scope);
CREATE INDEX idx_reports_evidence_id ON public.reports(evidence_id);
...
-- Reports - enhance existing RLS for persona scope
DROP POLICY IF EXISTS "Users can access reports by persona scope" ON public.reports;
CREATE POLICY "Users can access reports by persona scope" ON public.reports FOR ALL USING (
    user_id = auth.uid() OR 
    (persona_scope = 'advisor' AND user_id IN (
