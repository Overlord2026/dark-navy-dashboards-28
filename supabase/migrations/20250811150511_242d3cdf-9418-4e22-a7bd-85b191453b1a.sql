...
CREATE TABLE IF NOT EXISTS public.registry_records (
  ...
  field_confidence JSONB DEFAULT '{}', -- Per-field confidence scores
  identity_matches JSONB DEFAULT '{}', -- Name, DOB, license# matches
  license_status TEXT,
  license_number TEXT,
  license_expiry DATE,
...
CREATE TABLE IF NOT EXISTS public.trust_scores (
  ...
  score_history JSONB DEFAULT '[]', -- Array of {date, score, reason, streak_count}
  component_scores JSONB DEFAULT '{}', -- Identity, license, sanctions, references
  confidence_level NUMERIC(4,3) DEFAULT 0.500,
  tier TEXT NOT NULL DEFAULT 'bronze' CHECK (tier IN ('bronze', 'silver', 'gold', 'platinum')),
  tier_updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  access_permissions JSONB DEFAULT '[]',
  flags JSONB DEFAULT '[]', -- Warning flags, restrictions
  tenant_id UUID NOT NULL REFERENCES public.profiles(tenant_id),
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.reconciliation_logs (
  ...
  source_records JSONB NOT NULL, -- Array of conflicting registry_record IDs
...
-- Blockchain Anchoring Tables (Patent Feature)
...
-- Constraints
ALTER TABLE public.vetting_requests ADD CONSTRAINT unique_professional_pending_request 
  EXCLUDE (professional_id WITH =) WHERE (status IN ('pending', 'in_progress'));

ALTER TABLE public.trust_scores ADD CONSTRAINT unique_professional_current_score 
  EXCLUDE (professional_id WITH =) WHERE (updated_at IS NOT NULL);
