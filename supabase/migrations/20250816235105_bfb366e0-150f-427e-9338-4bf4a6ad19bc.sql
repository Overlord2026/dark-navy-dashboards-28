-- Security hardening: Fix function search_path settings
-- This migration addresses security warnings by setting explicit search_path on all functions

-- Functions that need explicit search_path = 'public', 'pg_temp'
ALTER FUNCTION public.activate_referral(uuid) SET search_path = 'public', 'pg_temp';
ALTER FUNCTION public.audit_franchise_payout_changes() SET search_path = 'public', 'pg_temp';  
ALTER FUNCTION public.audit_franchise_referral_changes() SET search_path = 'public', 'pg_temp';
ALTER FUNCTION public.calculate_audit_hash_sha3(uuid, uuid, text, text, text, text) SET search_path = 'public', 'pg_temp';
ALTER FUNCTION public.calculate_next_training_due_date(text, date) SET search_path = 'public', 'pg_temp';
ALTER FUNCTION public.can_user_access_organization(uuid) SET search_path = 'public', 'pg_temp';
...
ALTER FUNCTION public.enforce_professional_user_limit() SET search_path = 'public', 'pg_temp';
ALTER FUNCTION public.fn_touch_updated_at() SET search_path = 'public', 'pg_temp';
ALTER FUNCTION public.generate_vip_referral_code(uuid) SET search_path = 'public', 'pg_temp';
...
ALTER FUNCTION public.get_persona_data(text) SET search_path = 'public', 'pg_temp';
...
ALTER FUNCTION public.process_referral_reward(uuid, text, numeric) SET search_path = 'public', 'pg_temp';
...
ALTER FUNCTION public.schedule_follow_up(uuid, text) SET search_path = 'public', 'pg_temp';
...
ALTER FUNCTION public.validate_double_entry(uuid) SET search_path = 'public', 'pg_temp';
ALTER FUNCTION public.validate_plan_activation(uuid) SET search_path = 'public', 'pg_temp';

-- Critical functions that should stay with restricted search_path for security
-- vault_is_configured keeps empty search_path for security
-- check_extension_health keeps empty search_path for security
-- get_enhanced_security_metrics keeps current path for cross-schema access
-- calculate_agency_average_rating keeps current path for multi-table access
-- calculate_advisor_performance_metrics keeps current path for complex queries
