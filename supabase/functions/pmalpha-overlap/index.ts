...
// Overlap computation interfaces and types
 ...
 */
async function computeOverlap(input: OverlapInput, supabaseClient: any): Promise<OverlapResult> {
...
async function persistOverlapResults(
  userId: string,
...
serve(async (req) => {
  ...
  try {
    ...
    const overlapId = await persistOverlapResults(
      ...
      { fundIds, asOfDate, userId: user.id },
      overlapResults,
      supabaseClient
    );
