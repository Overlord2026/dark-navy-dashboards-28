...
export const PrivateEquityAccountsList = () => {
  ...
  const handleDeleteClick = (account: any) => {
    setAccountToDelete({ id: account.id, name: account.entity_name });
  ...
  if (accounts.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-muted-foreground">No private equity accounts added yet.</p>
          ...
          {accounts.map((account) => (
            <Card key={account.id} className="p-4">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h4 className="font-semibold text-lg">{account.entity_name}</h4>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>{account.entity_type}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Ownership:</span>
                  <span>{formatOwnership(account.ownership_percentage)}</span>
                </div>
              </div>
          ...
          <TableBody>
            {accounts.map((account) => (
              <TableRow key={account.id}>
                ...
                <TableCell>
                  {formatOwnership(account.ownership_percentage)}
                </TableCell>
