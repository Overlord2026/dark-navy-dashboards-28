...
export async function applyRules(
  ...
  let computedEntries = ledgerEntries;
  ...
  // Build insert rows and hash reasons content-free
  const rows = await Promise.all(
    computedEntries.map(async (entry) => ({
      ...entry,
      period,
      iar_id: iarId,
      reasons_hash: await inputs_hash({ reasons: entry.reasons }) // uses Web Crypto -> hex -> 'sha256:...'
    }))
  );
  ...
  await recordReceipt({
    ...
    period,
    iar_id: iarId,
    entries_count: computedEntries.length,
    policy_version: 'v1.0'
  });
