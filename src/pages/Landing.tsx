...
import { ArrowRight, Leaf, Sunset, Crown } from 'lucide-react';
      ...
      <div className="flex flex-col items-center min-h-screen bg-[#0A1F44] text-white">
        <div className="w-full px-4 py-8">
          <div className="text-center mb-10">
            <h1 className="text-2xl font-bold mb-3">Organize & Maximize</h1>
            <img 
          ...
          <div className="mt-8 space-y-6">
            {segments.map((seg) => {
              ...
              return (
                <div
                  key={seg.id}
                  className="p-6 rounded-lg bg-black/20 hover:bg-black/30 border border-[#D4AF37]/30 cursor-pointer transition-all hover:shadow-md flex flex-col"
                  onClick={() => handleSegmentClick(seg.id)}
                >
                  <div className="flex items-start justify-between mb-3">
                    <Icon className="text-[#D4AF37] h-6 w-6" />
                  ...
                  <Button 
                    className="mt-auto w-full bg-[#D4AF37] text-[#0A1F44] hover:bg-[#D4AF37]/90"
                    onClick={() => handleSegmentClick(seg.id)}
                  >
                    Explore {seg.label} <ArrowRight className="h-4 w-4 ml-1" />
                  </Button>
                </div>
              );
            })}
      ...
      <div className="flex flex-col items-center min-h-screen bg-[#0A1F44] text-white">
        <div className="w-full max-w-7xl px-4 py-12 md:py-20">
          <div className="landing-header text-center mb-16">
            <h1 className="text-4xl font-bold mb-6">Organize & Maximize</h1>
            <img 
              src="/lovable-uploads/3346c76f-f91c-4791-b77d-adb2f34a06af.png" 
              alt="Boutique Family Office" 
          ...
          <div 
            className="grid grid-cols-1 md:grid-cols-3 gap-8 px-4 mb-20" 
            style={{
              backgroundImage: 'radial-gradient(circle at 50% 50%, rgba(212, 175, 55, 0.1) 0%, rgba(10, 31, 68, 0) 70%)'
            }}
          >
            {segments.map((seg) => {
              const Icon = seg.icon;
              return (
                <div
                  key={seg.id}
                  className="p-8 rounded-lg bg-black/20 hover:bg-black/30 border border-[#D4AF37]/30 cursor-pointer transition-all duration-300 hover:shadow-lg hover:-translate-y-1 flex flex-col"
                >
                  <div className="flex items-start justify-between mb-4">
                    <Icon className="text-[#D4AF37] h-8 w-8" />
                  ...
                  <Button 
                    className="mt-auto w-full bg-[#D4AF37] text-[#0A1F44] hover:bg-[#D4AF37]/90 font-medium"
                    onClick={() => handleSegmentClick(seg.id)}
                  >
                    Explore {seg.label} <ArrowRight className="h-4 w-4 ml-1" />
                  </Button>
