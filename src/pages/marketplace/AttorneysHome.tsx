...
export default function AttorneysHome() {
  ...
  // Fetch attorneys from v_public_pros
  useEffect(() => {
    loadAttorneys();
  }, []);
  ...
  const loadAttorneys = async () => {
    try {
      ...
      toast({
        title: "Unable to load attorneys",
  ...
  // Get all unique tags from attorneys
  const allTags = useMemo(() => {
    const tagSet = new Set<string>();
    attorneys.forEach(attorney => {
      attorney.tags?.forEach(tag => tagSet.add(tag));
    });
  ...
  const filteredAttorneys = useMemo(() => {
    let filtered = attorneys.filter(attorney => {
      ...
      const matchesSearch = !searchQuery || 
        ...
        attorney.title?.toLowerCase().includes(searchLower) ||
        attorney.city?.toLowerCase().includes(searchLower);

      // Tag filter
      const matchesTags = selectedTags.length === 0 || 
        selectedTags.some(tag => attorney.tags?.includes(tag));
    ...
    return filtered;
  }, [attorneys, searchQuery, selectedTags, sortBy]);
  ...
  const handleInquirySubmit = async (data: {
    ...
    if (!inquiryModal.attorneyId) return { receiptHash: null };

    try {
      setInquiryLoading(true);
      
      const result = await recordInquiry({
        pro_id: inquiryModal.attorneyId,
  ...
  return (
    <div className="min-h-screen bg-bfo-black">
      <div className="container mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Estate Planning Attorneys</h1>
          <p className="text-white/70">Connect with experienced estate planning professionals</p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          ...
          {(searchQuery || selectedTags.length > 0) && (
            <div className="flex items-center gap-2 text-sm">
              <span className="text-white/70">Showing {filteredAttorneys.length} results</span>
              {(searchQuery || selectedTags.length > 0) && (
                <button
                  onClick={() => {
                    setSearchQuery('');
        ...
        {filteredAttorneys.length > 0 ? (
          ...
          <div className="bfo-card border border-bfo-gold/30 p-8 text-center">
            <p className="text-white mb-2">No attorneys found</p>
        ...
        <InquiryModal
          ...
          advisorName={inquiryModal.attorneyName}
