...
type PhaseWithNullableYearEnd = Omit<SwagPhase, 'yearEnd'> & { yearEnd: number | null };

const DEFAULT_PHASES: PhaseWithNullableYearEnd[] = [
  {
    id: 'income-now',
    name: 'Income Now',
    yearStart: 1,
...
export default function SWAGRetirementRoadmapPage() {
  ...
  const [phases] = useState<PhaseWithNullableYearEnd[]>(DEFAULT_PHASES);
  ...
  const handleRunAnalysis = async () => {
    try {
      ...
      const analysisInput: SwagRetirementAnalysisInput = {
        ...
        phases: phases.map(p => ({ 
          ...p, 
          allocation: (p as any).allocation || { stocks: 60, bonds: 30, alternatives: 10, cash: 0 },
          projection: (p as any).projection || { expectedReturn: 7.0, volatility: 15.0, projectedValue: 0, withdrawalCapacity: 0 },
        ...
        stress: { ltc: { startAge: 75, years: 3, monthlyCost: 5000 } },
        assets: [],
        assumptions: {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                {[
                  { key: 'profile', label: 'Enhanced Profile' },
                  { key: 'analysis', label: 'SWAG Analysis' },
                  { key: 'results', label: 'Phase Results' }
                ].map((step, index) => {
          ...
          {activeStep === 'results' && results && (
            <SwagPhaseManager
              phases={phases as unknown as SwagPhase[]}
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-medium">
                          {phase.yearStart}{phase.yearEnd != null ? `-${phase.yearEnd}y` : '+'}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
